...
The DHCP Server was configured on Windows Server 2025 Standard Evaluation and validated using a Windows 11 client. The implementation demonstrates centralized IPv4 address management, DHCP Scope configuration, Scope Options, address exclusions, lease management, and DHCP Reservations.
...
The DHCP Server role was installed on the Windows Server 2025 Standard Evaluation machine.
...
After completing this temporary allocation test, the DHCP Scope was restored to the final project configuration:
...
The retained captures validate one authorized DHCP server and one Windows 11 client. They do not demonstrate DHCP failover, high availability, or multi-subnet relay behavior.

Option 003 was intentionally omitted because the isolated network has no router or default gateway. During the exclusion test, the available range was temporarily constrained through `.191`; the scope was then restored to the final `.26`–`.245` pool before the phase was closed.
