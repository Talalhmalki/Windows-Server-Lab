...
10. Configure and validate storage reporting.
...
The retained screenshot verifies the following report-task properties:

- Scope: `F:\CompanyData\Departments`
- Selected reports: Large Files and Least Recently Accessed Files
- Large Files threshold: 50 MB
- Least Recently Accessed threshold: 3 days
- Output format: DHTML
- A configured task with next-run metadata

The captured properties do not expose a readable recurrence pattern. Accordingly, this report does not claim a weekly schedule or a specific weekday and time. This distinction preserves what the evidence proves without discarding the valid report configuration and generated results.
...
| Report Task | Task and next-run metadata visible | ✅ Passed |
...
- Report parameters
- DHTML reports
- Storage report task configuration
...
- Generate storage reports using the configured scope and parameters.
